...
n_parallel = 6; % run in parallel threads
err2weight = 1.5;% an attempt to improve error balance
...
for i = 1:n_genes
    CorrCoeff(i) = corr(gene_data_train(:,i), labels_train, 'Type','Spearman'); % Spearman - recommendation by A.Galatenko
end
figure; hist(CorrCoeff, 50);
fprintf('Computed correllations\n');
toc;
%% 4. Select which genes to use - find the top most correllated
